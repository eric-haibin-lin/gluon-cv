...
class ElasticLRScheduler(lr_scheduler.LRScheduler):
    r"""Learning Rate Scheduler
    ...
    warmup_mode : str
        ...
        Currently it supports 'linear' and 'constant'.
    adaptive: str
        How to adjust learning rate based on number of devices. Support both 'linear' and 'sqrt'
    """
    def __init__(self, mode, baselr, nsamples, nepochs,
                 step=(30, 60, 90), step_factor=0.1, targetlr=0, power=0.9,
                 warmup_epochs=0, warmup_lr=0, warmup_mode='linear',
                 adaptive=None, base_num_gpus=None):
        super(ElasticLRScheduler, self).__init__()
        assert(mode in ['step', 'poly', 'cosine'])
        assert(warmup_mode in ['linear', 'constant'])
        assert(adaptive in [None, 'linear', 'sqrt'])
        ...
        self.N = nepochs * nsamples
        self.warmup_N = warmup_epochs * nsamples
        self.adaptive = adaptive
        self.base_num_gpus = base_num_gpus
        if self.adaptive:
            assert base_num_gpus, "base num gpus must be provided in adaptive mode"
    ...
    def __call__(self, num_updates):
        """ This class incorrectly implements the LRScheduler interface in MXNet. """
        return self.learning_rate

    def update(self, i, epoch, num_gpus=None):
        T = epoch * self.nsamples + i
        assert(0 <= T <= self.N), (T, self.N)
        ...
        if self.warmup_epochs > epoch:
            ...
            if self.mode == 'step':
                ...
                raise NotImplementedError
        if self.adaptive == 'linear':
            self.learning_rate *= 1.0 * num_gpus / self.base_num_gpus
        if self.adaptive == 'sqrt':
            self.learning_rate *= sqrt(1.0 * num_gpus / self.base_num_gpus)
